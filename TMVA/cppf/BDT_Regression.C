...
void BDT_Regression( TString myMethodList = "" )
{
   ...
   for(int i = 0; i < Digis.size() ; i++ ){
    dataloader->AddVariable( Digis[i].c_str(), Digis[i].c_str(), "units", 'S' );
   }

   // example dataloader->AddVariable( "Deltatheta14", "Deltatheta14", "units", 'S' );
...
//   dataloader->AddVariable( "Cppf_theta3", "Cppf_theta3", "units", 'S' );
   ...
   for(int i = 0; i < Digis_Pt.size() ; i++ ){
    if(i!=0) continue;
    dataloader->AddTarget(Digis_Pt[i].c_str() ); 
   }
   ...
   TTree *regTree = (TTree*)input->Get("BDTCPPF/bdt_cppf");
   ...
   TCut mycut = "NCPPF >= 2"; // for example: TCut mycut = "abs(var1)<0.5 && abs(var2-0.5)<1";

   // tell the DataLoader to use all remaining events in the trees after training for testing:
   dataloader->PrepareTrainingAndTestTree( mycut,
                                         "nTrain_Regression=20000:nTest_Regression=2800:SplitMode=Random:NormMode=NumEvents:!V" );
   //
   //     dataloader->PrepareTrainingAndTestTree( mycut,
...
*/
   ...
   if (Use["BDT"])
     factory->BookMethod( dataloader,  TMVA::Types::kBDT, "BDT",
                           "!H:!V:NTrees=2000:MinNodeSize=0.1%:BoostType=AdaBoost:SeparationType=GiniIndex:nCuts=20:PruneMethod=CostComplexity:PruneStrength=30:MaxDepth=3" );

   if (Use["BDTG"])
     factory->BookMethod( dataloader,  TMVA::Types::kBDT, "BDTG",
                 "!H:!V:NTrees=2000::BoostType=Grad:Shrinkage=0.1:UseBaggedBoost=True:BaggedSampleFraction=0.8:nCuts=20:MaxDepth=3" );
   // --------------------------------------------------------------------------------------------------
