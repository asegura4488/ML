...
void fill(){
  ...
  //Histo.push_back("BDT");
  //Histo.push_back("BDTG");
  //Histo.push_back("MLP");
  Histo.push_back("h7");
  Histo.push_back("h8");
  Histo.push_back("h9");
  
  Legends_.push_back("BDT");
  Legends_.push_back("BDTG");
  Legends_.push_back("MLP");
...
void plot(){
  ...
  for(int k = 0; k < Histo.size(); k++) {
    ...
    if(k==0){ 
      ...
      HISTO[k]->GetXaxis()->SetTitle("BDT_{p_{T}}/Real_{p_{T}}"); 
      HISTO[k]->Draw();
    }
